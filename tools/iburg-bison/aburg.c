/* modified by Andreas Krall on August 30 1995 for correct types */
...
static char rcsid[] = "$Id: iburg.c,v 1.2 93/03/27 11:35:48 drh Exp $";
...
int main(int argc, char *argv[]) {
	...
	if (outfp == NULL)
		outfp = stdout;
	print("typedef struct %Pstate *STATEPTR_TYPE;\n\n");
	yyparse();
	if (start)
		ckreach(start);
...
static void emitclosure(Nonterm nts) {
	...
	for (p = nts; p; p = p->link)
		if (p->chain)
			print("static void %Pclosure_%S(STATEPTR_TYPE, int);\n", p);
	print("\n");
	for (p = nts; p; p = p->link)
		if (p->chain) {
			Rule r;
			print("static void %Pclosure_%S(STATEPTR_TYPE p, int c) {\n", p);
...
static void emitfuncs(void) {
	print("int %Pop_label(NODEPTR_TYPE p) {\n"
	      ...
	      "%1return OP_LABEL(p);\n}\n\n");
	print("STATEPTR_TYPE %Pstate_label(NODEPTR_TYPE p) {\n"
...
static void emitlabel(Nonterm start) {
	...
	print("%2STATE_LABEL(p) = %Pstate(OP_LABEL(p),\n"
	      ...
	      "%1}\n}\n\n");
	print("STATEPTR_TYPE %Plabel(NODEPTR_TYPE p) {\n%1%Plabel1(p);\n"
	      "%1return STATE_LABEL(p)->rule.%P%S ? STATE_LABEL(p) : 0;\n"
	      "}\n\n", start);
}
...
static void emitrule(Nonterm nts) {
	...
	print("int %Prule(STATEPTR_TYPE state, int goalnt) {\n"
	      "%1%Passert(goalnt >= 1 && goalnt <= %d, PANIC(\"Bad goal nonterminal %%d in %Prule\\n\", goalnt));\n"
	      "%1if (!state)\n%2return 0;\n%1switch (goalnt) {\n", ntnumber);
	for (p = nts; p; p = p->link)
		print("%1case %P%S_NT:"
	          "\n%2return %Pdecode_%S[state->rule.%P%S];\n", p, p, p);
...
static void emitstate(Term terms, Nonterm start, int ntnumber) {
	...
	print("STATEPTR_TYPE %Pstate(int op, STATEPTR_TYPE left, STATEPTR_TYPE right) {\n%1int c;\n"
	      "%1STATEPTR_TYPE p, l = left, r = right;\n\n%1");
	if (!Tflag)
		print("if (%Parity[op] > 0) ");
	print("{\n%2p = (STATEPTR_TYPE)ALLOC(sizeof *p);\n"
