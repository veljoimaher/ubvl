char rcsid_plank[] = "$Id: plank.c,v 4.9 1992/06/10 04:52:07 todd Exp $";
...
static void enterStateMap ARGS((PlankMap, DeltaPtr, int, int *));
...
doDimPmaps(op) Operator op;
{
	...
	case 1:
		...
		break;
	case 2:
		if (op->table->dimen[0]->map->count == (2-safely) && op->table->dimen[1]->map->count == (2-safely)) {
			...
			for (i = (1-safely); i < op->table->dimen[0]->map->count; i++) {
				...
				for (j = (1-safely); j < op->table->dimen[1]->map->count; j++) {
					...
					if ((*ts)->num > 0) {
						diff = (*ts)->newNum - op->baseNum +safely;
					} else {
						diff = safely ? 0 : -1;
					}
					fprintf(outfile, "%5d", diff);
				}
				fprintf(outfile, "}\t/* row %d */", i-(1-safely));
			}
			fprintf(outfile, "\n};\n");
		}
		break;
...
doPlankLabelMacrosSafely(op) Operator op;
{
	...
	case 0:
		...
		fprintf(outfile, "\t%d\n", op->table->transition[0]->newNum+safely);
		break;
	case 1:
